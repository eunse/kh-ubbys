...
<!-- 탈퇴 회원관리 -->
	...
	<div class="container py-5">
		...
		<table class="table table-striped table-hover w-100">
			...
			<tbody>
				<c:forEach var="un" items="${unRegUserList}">
					<tr>
						<td><input type="checkbox"></td>
						<td>${un.userNo }</td>
						<td><a href="#">${un.userEmail}</a></td>
						<td>${un.userNickname}</td>
						<td><fmt:formatDate var="userRegdate"
								value="${un.userUnRegdate}" pattern="yyyy-MM-dd" /> <fmt:formatDate
								var="today" value="<%=new java.util.Date()%>"
								pattern="yyyy-MM-dd" /> <c:choose>
								<%-- 글 작성일이 오늘이 아닐 경우 --%>
								<c:when test="${un.userUnRegdate != today}"> ${un.userUnRegdate}
		                    </c:when>


								<%-- 글 작성일이 오늘일 경우 --%>
								<c:otherwise>
									<fmt:formatDate value="${un.userUnRegdate}" pattern="yyyy-MM" />
								</c:otherwise>
							</c:choose></td>
						<td><a
